...
test('prefix search', () => {
  expect(createTrie().prefixSearch("abracadabra"))
    .toEqual([{ value: 500, length: 2 }, { value: 300, length: 4 }]);
  expect(createTrie().prefixSearch("abda"))
    .toEqual([{ value: 500, length: 2 }, { value: 200, length: 4 }]);
  expect(createTrie().prefixSearch("abdaa"))
    .toEqual([{ value: 500, length: 2 }, { value: 200, length: 4 }]);
  expect(createTrie().prefixSearch(""))
    .toEqual([]);
  expect(createTrie().prefixSearch("ca"))
    .toEqual([]);
  expect(createTrie().prefixSearch("cab"))
    .toEqual([{ value: 400, length: 3 }]);
  expect(createTrie().prefixSearch("cabaaaaa"))
    .toEqual([{ value: 400, length: 3 }, { value: 0, length: 5 }]);
});
...
test('serialize/deserialize', () => {
  ...
  expect(trie.prefixSearch("abracadabra"))
    .toEqual([{ value: 500, length: 2 }, { value: 300, length: 4 }]);
});